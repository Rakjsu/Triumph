...
impl<M> AchievementHelper<'_, M> {
    ...
    pub fn get_achievement_icon(&self) -> Option<Vec<u8>> {
        unsafe {
            ...
            // Patched: removed assert_eq!(width, 64) - support any icon size
            let buf_size = (width * height * 4) as usize;
            let mut dest = vec![0u8; buf_size];
            if !sys::SteamAPI_ISteamUtils_GetImageRGBA(utils, img, dest.as_mut_ptr(), buf_size as i32) {
                return None;
            }
            Some(dest)
        }
